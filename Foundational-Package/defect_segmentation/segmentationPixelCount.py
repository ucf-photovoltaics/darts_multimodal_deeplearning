...
img_path     = output_root / "CellsCropped"         # folder where images are
model_path   = current_file.parent / "models"        # folder where model is stored
model_name   = "model_97.pth"                        # trained model name
save_path    = output_root / "PixelVisuals"          # location to save figures
defect_dir   = output_root / "PixelDefects"          # save defect percentage jsons
csv_path     = output_root / "pixel_analysis.csv"    # CSV analysis

defect_per   = True                                  # turn on if you want defect percentages
use_gpu      = torch.cuda.is_available()              # determines if GPU can be used
##################################################
################ model parameters ################
